...
class SignalDetector:
    ...
    def __init__(self):
        ...
        self.AUTO_CLOSE_THRESHOLD_LOSS = -3.0  # USD
    ...
    def _autobot_open_position(self, symbol: str, position: str, mark_price: float, current_candle: datetime):
        ...
        if db_success:
            ...
            # Mulai trailing stop monitoring - PERUBAHAN: Menggunakan trailing_stops yang baru
            self.trailing_stops[binance_order_id] = {
                'max_profit': 0.0,
                'open_price': mark_price,
                'symbol': symbol,
                'position': position,
                'quantity': final_quantity,
                'is_manual': False  # Tandai sebagai order autobot
            }
    ...
    def _monitor_trailing_stop(self, binance_order_id: str):
        """Thread untuk memantau trailing stop untuk semua order (manual dan autobot) dalam USD"""
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                with self.auto_close_lock:
                    if binance_order_id not in self.trailing_stops:
                        break

                    trail_data = self.trailing_stops[binance_order_id]
                    ...
                    is_manual = trail_data.get('is_manual', False)
                ...
                if is_manual:
                    min_target_profit = 2.5  # Target profit minimal untuk order manual
                    trailing_distance = 0.5  # Jarak trailing untuk order manual
                else:
                    min_target_profit = self.AUTOBOT_MIN_TARGET_PROFIT
                    trailing_distance = self.AUTOBOT_TRAILING_DISTANCE

                # Jika mencapai target minimum, mulai trailing
                if current_profit >= min_target_profit:
                    # Update profit maksimum jika melebihi
                    if current_profit > max_profit:
                        with self.auto_close_lock:
                            self.trailing_stops[binance_order_id]['max_profit'] = current_profit
                    ...
                    if current_profit <= (max_profit - trailing_distance):
                        logger.info(f"Trailing stop dipicu! Menutup posisi {binance_order_id}")
                        self._close_position_by_trailing(binance_order_id, mark_price)
                        break
                # PERBAIKAN: Tambahkan logika jika profit di bawah minimum
                elif current_profit < min_target_profit:
                    # Reset max profit jika profit turun di bawah minimum
                    with self.auto_close_lock:
                        self.trailing_stops[binance_order_id]['max_profit'] = 0.0
        ...
        # Hapus dari tracking
        with self.auto_close_lock:
            if binance_order_id in self.trailing_stops:
                del self.trailing_stops[binance_order_id]
        logger.info(f"Trailing stop dihentikan untuk order {binance_order_id}")

    def _close_position_by_trailing(self, binance_order_id: str, current_price: float):
        """Tutup posisi berdasarkan trailing stop untuk semua order"""
        try:
            ...
            if close_result['success']:
                ...
                logger.info(f"Trailing stop: Posisi {binance_order_id} ditutup @ {close_result['price']}")
                return True
            else:
                logger.error(f"Trailing stop: Gagal menutup posisi {binance_order_id}")
                return False

        except Exception as e:
            logger.error(f"Error closing position by trailing: {e}")
            return False
            ...
            try:
                ...
                if status in ['PARTIALLY_FILLED', 'FILLED']:
                    ...
                    if executed_qty > 0:
                        ...
                        with self.auto_close_lock:
                            if binance_order_id in self.trailing_stops:
                                self.trailing_stops[binance_order_id]['open_price'] = executed_price
                        ...
                        if update_success:
                            logger.info(f"Successfully synced price for order {binance_order_id} @ {executed_price}")
                        else:
                            logger.warning(f"Failed to update price for order {binance_order_id}")

                        order_filled = True
                        break
                elif status in ['CANCELED', 'EXPIRED']:
                    logger.warning(f"Order {binance_order_id} canceled/expired")
                    break

            except Exception as e:
                logger.error(f"Error syncing order price: {e}")

        if not order_filled:
            logger.warning(f"Order {binance_order_id} not filled after {max_attempts} attempts")
            ...
            try:
                ...
                if 'depth' in data.get('stream', ''):
                    ...
                    if symbol in self.symbols:
                        with self.data_lock:
                            self.order_books[symbol] = {'bids': stream_data.get('b', []), 'asks': stream_data.get('a', []), 'timestamp': datetime.utcnow()}
    ...
    def submit_order(self):
        ...
        try:
            ...
            enable_trailing_stop = data.get('enable_trailing_stop', False)  # PERUBAHAN: Tambah parameter trailing stop
            ...
            # PERUBAHAN: Mulai trailing stop monitoring untuk order manual jika diaktifkan
            if enable_trailing_stop:
                self.trailing_stops[binance_order_id] = {
                    'max_profit': 0.0,
                    'open_price': db_price,
                    'symbol': symbol,
                    'position': "LONG" if side == OrderSide.BUY else "SHORT",
                    'quantity': float(formatted_quantity),
                    'is_manual': True  # Tandai sebagai order manual
                }
                ...
                logger.info(f"Trailing stop diaktifkan untuk order manual {binance_order_id}")
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if status in ['PARTIALLY_FILLED', 'FILLED']:
                    ...
                    # Hapus dari trailing stops jika ada
                    with self.auto_close_lock:
                        if binance_order_id in self.trailing_stops:
                            del self.trailing_stops[binance_order_id]
    ...
    def _pnl_monitor(self):
        """
        [FIXED] Monitor untuk menutup posisi secara otomatis berdasarkan kerugian.
        Logika PnL diperbaiki untuk menghitung untung/rugi dengan benar.
        """
        while not self.shutdown_event.is_set():
            try:
                time.sleep(3)
                ...
                for order in orders_data.get('data', []):
                    ...
                    # Validasi data dasar
                    if not all([symbol, order_id, position]) or open_price <= 0 or quantity <= 0:
                        continue

                    # Dapatkan harga pasar saat ini
                    current_price = mark_prices_snapshot.get(symbol)
                    if not current_price:
                        continue

                    # --- PERBAIKAN LOGIKA PNL ---
                    # Hitung PnL (akan negatif jika rugi)
                    if position == 'LONG':
                        pnl = (current_price - open_price) * quantity
                    else:  # SHORT
                        pnl = (open_price - current_price) * quantity

                    # Perkirakan biaya trading (misal: 0.04% untuk buka dan 0.04% untuk tutup)
                    fee = (open_price * quantity * 0.0008)
                    net_pnl = pnl - fee

                    # Cek apakah PnL bersih mencapai ambang batas kerugian
                    if net_pnl <= self.AUTO_CLOSE_THRESHOLD_LOSS:
                        with self.auto_close_lock:
                            if order_id in self.orders_in_process:
                                continue
                            self.orders_in_process.add(order_id)

                            logger.warning(f"Stop loss triggered for {order_id}: PnL={net_pnl:.4f} USD")

                            # Jalankan penutupan posisi di thread terpisah
                            threading.Thread(
                ...
                logger.error(f"Stop Loss Monitor error: {e}", exc_info=True)
    ...
    def cancel_order(self):
        ...
        try:
            ...
            # 3. Hapus dari trailing stops jika ada
            with self.auto_close_lock:
                if order_id in self.trailing_stops:
                    del self.trailing_stops[order_id]
        ...
        with self.data_lock:
            for symbol in self.symbols:
                ...
                asset = {
                    ...
                    'posisi_sch': f"{signal_info.get('posisi', 'HOLD')}",
		    'timers': f"{signal_info.get('timer', '')}",
                    # Posisi Sign: menggunakan data dari database (posisi + signal_score)
                    # PERBAIKAN: Format menjadi 'POSISI / SKOR' dan bulatkan skor
                    'posisi_sign': f"{signal_info.get('label_action', 'HOLD')}",
