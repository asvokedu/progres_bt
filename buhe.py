...
class SignalDetector:
    ...
    def submit_order(self):
        """Endpoint untuk menerima order dari frontend dengan logika kuantitas yang disempurnakan"""
        try:
            data = request.get_json()
            logger.info(f"Menerima request order: {data}")

            required_fields = ['symbol', 'side', 'order_type', 'leverage']
            ...
            # Dapatkan info simbol (aturan presisi, dll)
            symbol_info = self.symbol_info_map.get(symbol)
            if not symbol_info:
            ...
            effective_price = 0
            if order_type == OrderType.LIMIT and price_from_user:
                effective_price = price_from_user
            else:
                effective_price = self.mark_prices.get(symbol)
            ...
            # Hitung kuantitas dari persentase saldo
            notional_percent = float(data.get('notional_percent', 0))
            if notional_percent > 0:
                with self.data_lock:
                    balance = self.account_balance_cache.get('USDT', 0)
                if balance <= 0:
                    return jsonify({'success': False, 'msg': 'Saldo tidak tersedia atau nol'}), 400
                
                notional_value = (balance * notional_percent) / 100
                quantity_float = notional_value / effective_price
            else:
                return jsonify({'success': False, 'msg': 'Persentase saldo harus lebih dari 0'}), 400
            
            # Cek minNotional
            if (quantity_float * effective_price) < min_notional:
                quantity_float = (min_notional * 1.05) / effective_price
                logger.warning(f"Notional awal dibawah minimum. Kuantitas disesuaikan menjadi: {quantity_float}")

            # --- PERBAIKAN UTAMA: FORMAT KUANTITAS DAN HARGA ---
            ...
            logger.info(f"Final formatted values -> Quantity: {formatted_quantity}, Price: {formatted_price}")
            ...
            self._refresh_signal_data()
            return jsonify({'success': True, 'order_id': binance_order_id, 'msg': 'Order submitted successfully!'})
